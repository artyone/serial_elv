...
class Elvis(object):
    ...
    def set_fout(self,
        ...
        all_dPh_data = self.__get_data_for_registers(frequency, amplitude)

        # Массив данных не используется
        # if channel_number == 1:
        #     #all_dPh_data += self.CH1[profile_index]['data'].values()
        #     all_dPh_data.extend([0, amplitude, 0])
        # else:
    ...
    def __exec_command(
        ...
        with Serial(baudrate=baudrate, bytesize=EIGHTBITS, timeout=0.2) as ser:
            ...
            for ch, data in zip(registers, datas):
                ...
                self.command.append(
                    hex(int.from_bytes(send_message, byteorder='big')))
    ...
    def status_CH1_to_ON(self, port_index, timeout, baudrate):
        """
        Функция смены статуса 1 канала на положение ВКЛ.
        ...
        сразу 2 каналов.   
        """
        if not self.state_CH2:
            answer = self.__exec_command(
                [self.CTR], [0x1000], port_index, timeout, baudrate)
            self.state_CH1 = True
            return answer
        if self.state_CH2:
            answer = self.__exec_command(
                [self.CTR], [0x3000], port_index, timeout, baudrate)
            self.state_CH1 = True
            return answer
        return False

    def status_CH1_to_OFF(self, port_index, timeout, baudrate):
        if not self.state_CH2:
            answer = self.__exec_command(
                [self.CTR], [0x0000], port_index, timeout, baudrate)
            self.state_CH1 = False
            return answer
        if self.state_CH2:
            answer = self.__exec_command(
                [self.CTR], [0x2000], port_index, timeout, baudrate)
            self.state_CH1 = False
            return answer
        return False

    def status_CH2_to_ON(self, port_index, timeout, baudrate):
        """
        Функция смены статуса 2 канала положение ВКЛ.
        Статус канала может быть либо ВКЛ, либо ВЫКЛ, но в связи с тем,
        что на микросхеме не реализован вывод данных, то весь процесс
        сохраняется только в сессии программы.
    ...
    def status_CH2_to_OFF(self, port_index, timeout, baudrate):
        if not self.state_CH1:
            answer = self.__exec_command(
                [self.CTR], [0x0000], port_index, timeout, baudrate)
            self.state_CH2 = False
    ...
    def set_sel_reg(self, ch_1, ch_2, port_index, timeout, baudrate):
        ...
        answer = self.__exec_command(
            [self.SEL_REG], [data], port_index, timeout, baudrate)
        return answer
    ...
    def set_clear(self, port_index, timeout, baudrate):
        """Очистка"""
        answer = self.__exec_command(
            [self.CLR], [0x000F], port_index, timeout, baudrate)
        self.state_CH1 = False
        self.state_CH2 = False
        return answer
